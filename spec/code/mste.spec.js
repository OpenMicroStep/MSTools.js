...
*/
    ...
    function deepEqual(a, b) {
        ...
        if (typeof a === "object") {
            ...
            for(k in a) {
                if (a.hasOwnProperty(k) && (!b.hasOwnProperty(k) || !deepEqual(a[k], b[k]))) {
                    return false;
                }
            }
            for(k in b) {
                if (b.hasOwnProperty(k) && (!a.hasOwnProperty(k) || !deepEqual(a[k], b[k]))) {
                    return false;
                }
            }
            return true;
    ...
    function test_mste(mste, value) {
        ...
        expect(function() { decoded= MSTools.MSTE.parse(mste, { crc: false }); }).not.toThrow(undefined, { mste: mste, value: value});
        expect(decoded).toBeDeepEqual(value, { mste: mste, value: value, decoded: decoded});
        expect(function() { encoded = MSTools.MSTE.stringify(value); }).not.toThrow(undefined, { mste: mste, value: value});
        expect(function() { redecoded = MSTools.MSTE.parse(encoded, { crc: false }); }).not.toThrow(undefined, { mste: mste, value: value, encoded: encoded});
        expect(redecoded).toBeDeepEqual(value, { mste: mste, value: value, encoded: encoded, redecoded: redecoded});
    }
    ...
    it('empty data',   function() { test_mste("[\"MSTE0102\",6,\"CRCE62DFB74\",0,0,4]", new MSData()); });
    ...
    it('data',         function() { test_mste("[\"MSTE0102\",7,\"CRC4964EA3B\",0,0,25,\"YTF6MmUzcjR0NA==\"]", new MSData("a1z2e3r4t4")); });
